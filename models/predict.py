...
def predict(texts, is_file, model_file, state_path, predict_path, return_full_dict=False, scheme="IOBES", batch_size=5, device="gpu:0", seed=4, criteria='loss', criteria2='None', patience=10, mode='min'):
    """
    ...
    ner_data.preprocess(texts, split_dict, is_file=is_file,
                        annotated=False, sentence_level=False, shuffle=False, seed=seed)
    ner_data.create_dataloaders(
        batch_size=batch_size, shuffle=False, seed=seed)
    bert_ner = BERTNER(model_file=model_file,
                       classes=ner_data.classes, scheme=scheme, seed=seed)
    bert_ner_trainer = NERTrainer(
        bert_ner, device, criteria, criteria2, patience, mode)
