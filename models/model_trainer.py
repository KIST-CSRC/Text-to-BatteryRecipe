...
class EarlyStopping:
    def __init__(self, patience, mode, delta=0.0, verbose=True):
        """
        patience (int): Number of epochs to wait without improvement (default: 3)
        delta (float): Minimum change to qualify as an improvement (default: 0.0)
        mode (str): Criteria for improvement ('min' or 'max', default: 'min')
        verbose (bool): Whether to output messages (default: True)
        """
...
class NERTrainer(object):
    ...
    def __init__(self, model, device, criteria, criteria2, patience, mode):
        '''
    ...
    def load_state(self, state_path, optimizer=True):
        ...
        checkpoint = torch.load(
            state_path, map_location=torch.device(self.device))
    ...
    def save_state_to_cache(self, key, optimizer=True):
        ...
        self.state_cacher.store(
            'model_state_dict_{}'.format(key), self.model.state_dict())
        # if optimizer, cache optimizer state dictionary with key
        if optimizer:
            self.state_cacher.store('optimizer_state_dict_{}'.format(
                key), self.optimizer.state_dict())
    ...
    def load_state_from_cache(self, key, optimizer=True):
        ...
        self.model.load_state_dict(self.state_cacher.retrieve(
            'model_state_dict_{}'.format(key)))
        # if optimizer, load optimzier state dictionary with key
        if optimizer:
            self.optimizer.load_state_dict(self.state_cacher.retrieve(
                'optimizer_state_dict_{}'.format(key)))
    ...
    def init_optimizer(self, optimizer_name, elr, tlr, clr, weight_decay):
        ...
        optimizer = optimizers[optimizer_name]([{'params': [p for n, p in bert_embeddings_params if not any(nd in n for nd in no_decay)], 'lr': elr, 'weight_decay': weight_decay},
                                                {'params': [p for n, p in bert_embeddings_params if any(
                                                    nd in n for nd in no_decay)], 'lr': elr, 'weight_decay': 0.0},
                                                {'params': [p for n, p in bert_encoder_params if not any(
                                                    nd in n for nd in no_decay)], 'lr': tlr, 'weight_decay': weight_decay},
                                                {'params': [p for n, p in bert_encoder_params if any(
                                                    nd in n for nd in no_decay)], 'lr': tlr, 'weight_decay': 0.0},
                                                {'params': [p for n, p in classifier_params if not any(
                                                    nd in n for nd in no_decay)], 'lr': clr, 'weight_decay': weight_decay},
                                                {'params': [p for n, p in classifier_params if any(
                                                    nd in n for nd in no_decay)], 'lr': clr, 'weight_decay': 0.0},
                                                {'params': [p for n, p in crf_params if not any(
                                                    nd in n for nd in no_decay)], 'lr': clr, 'weight_decay': weight_decay},
                                                {'params': [p for n, p in crf_params if any(nd in n for nd in no_decay)], 'lr': clr, 'weight_decay': 0.0}])
        if la:
            self.optimizer = Lookahead(
                base_optimizer=optimizer, k=10, alpha=0.5)
        else:
    ...
    def init_scheduler(self, n_epoch, bert_unfreeze, function_name='exponential'):
        ...
        self.scheduler = LambdaLR(
            self.optimizer, lr_lambda=functions[function_name], verbose=True)
    ...
    def construct_valid_inputs(self, inputs):
        ...
        valid = {key: np.zeros((batch_size, max_len), dtype=int) for key in [
    ...
    def process_labels(self, inputs, prediction_ids):
        ...
        valid_input_ids, valid_label_ids, valid_attention_mask = self.construct_valid_inputs(
            inputs)
        ...
        labels = [[self.model.classes[vli] for vli, vii in zip(vlis, viis) if vii not in (
            self.cls_dict['id'], self.sep_dict['id'])] for vlis, viis in zip(valid_label_ids, valid_input_ids)]
        predictions = [[self.model.classes[pi] for pi, vii in zip(pis, viis) if vii not in (
            self.cls_dict['id'], self.sep_dict['id'])] for pis, viis in zip(prediction_ids, valid_input_ids)]
    ...
    def process_ids(self, ids, input_ids, attention_mask, valid_mask, prediction_ids):
        ...
        toks = [self.model.tokenizer.convert_ids_to_tokens(
            sequence) for sequence in input_ids]
        # convert label ids to classifications for each entry
        lbls = [[self.model.classes[l] for l in sequence]
                for sequence in prediction_ids]
            ...
            for j in range(len(valid_mask[i])):
                ...
                if valid_mask[i][j] == 1:
                    ...
                    # if valid token and not special token
                        ...
                        slbl[k].append(lbls[i][v] if lbls[i][v]
                                       == 'O' else lbls[i][v].split('-')[1])
    ...
    def process_summaries(self, annotations):
        # class types
        class_types = sorted(list(
            set([class_.split('-')[1] for class_ in self.model.classes if class_ != 'O'])))
        # for entry
        for annotation in annotations:
            # initialize empty dictionary of extracted entities
            entry_entities = {class_type: set([])
                              for class_type in class_types}
            # for sentence in entry
            for sentence in annotation['tokens']:
                # extract beginning indices for entities
                entity_idx = [k for k in range(
                    len(sentence)) if sentence[k]['annotation'] != sentence[k-1]['annotation']]
                ...
                for k in range(len(entity_idx)):
                    ...
                    if k != len(entity_idx)-1:
                        ...
                        if sentence[entity_idx[k]]['annotation'] != 'O':
                            ...
                            entry_entities[sentence[entity_idx[k]]['annotation']].add(
                                ' '.join([sentence[u]['text'] for u in range(entity_idx[k], entity_idx[k+1])]))
            # append entry entity dictionary
            annotation['entities'] = {class_type: sorted(
                list(set(entry_entities[class_type]))) for class_type in class_types}
        return annotations
    ...
    def iterate_batches(self, epoch, n_epoch, iterator, mode):
        ...
        if mode == 'predict':
            prediction_results = {'ids': [], 'pts': [], 'input_ids': [
            ], 'attention_mask': [], 'valid_mask': [], 'prediction_ids': []}
        ...
        for batch in batch_range:
            ...
            if mode == 'predict':
                for key in prediction_results.keys():
                    if key == 'ids':
                        ...
                        prediction_results[key].extend(
                            list(inputs[key].cpu().numpy()))
            ...
            if mode != 'predict':
                ...
                report = classification_report(
                    batch_results['labels'], batch_results['predictions'], mode=self.metric_mode, scheme=self.metric_scheme, output_dict=True)
                # add accuracy score to report
                report['accuracy'] = accuracy_score(
                    batch_results['labels'], batch_results['predictions'])
                # add loss to report
                report['loss'] = loss.item()
                # append report to metrics
                metrics.append(report)
                # calculate rolling means for precision, recall and f1-score
                means = {m: np.mean([r['micro avg'][m] for r in metrics]) for m in [
            ...
            if mode != 'predict':
                ...
                info = (self.past_epoch+epoch+1, self.past_epoch+n_epoch, mode,
                        means['loss'], means['accuracy'], means['precision'], means['recall'], means['f1-score'])
            # if mode is predict
            else:
    ...
    # self.train_evaluate_epoch(epoch, n_epoch, valid_iter, 'valid')
    def train_evaluate_epoch(self, epoch, n_epoch, iterator, mode):
        ...
        # if testing
            ...
            with torch.no_grad():
                # evaluate all of the batches and collect the metrics
                metrics, test_results = self.iterate_batches(
                    epoch, n_epoch, iterator, mode)
        # if predicting
        elif mode == 'predict':
            # make sure the model is set to evaluate
            self.model.eval()
            ...
            with torch.no_grad():
                ...
                prediction_results = self.iterate_batches(
                    epoch, n_epoch, iterator, mode)
        # return statements
        if mode == 'test':
            return metrics, test_results
    ...
    def train(self, n_epoch, train_iter, valid_iter, embedding_unfreeze, encoder_schedule, scheduling_function, save_dir=None, use_cache=False):
        ...
        encoder_unfreeze = next(
            (i for i, n in enumerate(encoder_schedule) if n), n_epoch)
        ...
        for epoch in range(n_epoch):
            ...
            for layer in range(encoder_schedule[epoch]):
                ...
                expanded_encoder_schedule['epoch_{}'.format(
                    epoch)].append(last_encoder_layer)
        ...
        for epoch in range(n_epoch):
            ...
            train_metrics = self.train_evaluate_epoch(
                epoch, n_epoch, train_iter, 'train')
            # append history
            self.epoch_metrics['training']['epoch_{}'.format(
                self.past_epoch+epoch)] = train_metrics

            valid_metrics = self.train_evaluate_epoch(
                epoch, n_epoch, valid_iter, 'valid')
            if self.criteria2 == 'None':
                es(valid_metrics[1][self.criteria])
            else:
                es(valid_metrics[1][self.criteria][self.criteria2])
            if es.early_stop:
                break
                # append_history
            self.epoch_metrics['validation']['epoch_{}'.format(
                self.past_epoch+epoch)] = valid_metrics
            # save best
            validation_f1 = np.mean(
                [batch_metrics['micro avg']['f1-score'] for batch_metrics in valid_metrics])
    ...
    def test(self, test_iter, test_path=None, state_path=None):
        ...
        metrics, test_results = self.train_evaluate_epoch(
            0, 1, test_iter, 'test')
        # save the test metrics and results
        if test_path is not None:
            with open(test_path, 'w') as f:
                f.write(json.dumps(
                    {'metrics': metrics, 'results': test_results}, indent=2, cls=NpEncoder))
    ...
    def merge_split_entries(self, prediction_results):
        ...
        merged_prediction_results = {key: []
                                     for key in prediction_results.keys() if key != 'pts'}
        for id in unique_ids:
            merged_prediction_results['ids'].append(id)
            id_pts_ind = np.where(prediction_results['ids'] == id)[0]
            id_pts_ind = id_pts_ind[np.argsort(
                np.array(prediction_results['pts'])[id_pts_ind])]
            for i, u in enumerate(id_pts_ind):
                if i == 0:
                    for key in prediction_results.keys():
                        if key not in ['ids', 'pts']:
                            merged_prediction_results[key].append(
                                list(prediction_results[key][u]))
                else:
                    for key in prediction_results.keys():
                        if key not in ['ids', 'pts']:
                            merged_prediction_results[key][-1].extend(
                                list(prediction_results[key][u]))
    ...
    def predict(self, predict_iter, original_data=None, state_path=None, predict_path=None, return_full_dict=False):
        ...
        prediction_results = self.train_evaluate_epoch(
            0, 1, predict_iter, 'predict')
        ...
        if original_data is not None:
            ...
            for original in original_data:
                ...
                output_annotation = {
                    'id': original['id'], 'meta': original['meta'], 'tokens': annotation['tokens']}
