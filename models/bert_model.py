...
class BERTNER(BertPreTrainedModel):
    ...
    def build_model(self):
        ...
        if 'matbert' in self.model_file:
            self.bert = BertModel(self.config).from_pretrained(self.model_file)
        else:
            self.bert = AutoModelForMaskedLM.from_pretrained(self.model_file)
            self.bert = BertModel(self.config).from_pretrained(self.model_file)
        ...
        self.crf = CRF(classes=self.classes,
                       scheme=self.scheme, batch_first=True)
    ...
    def forward(self, input_ids, label_ids=None, attention_mask=None, valid_mask=None, return_logits=False, device='cpu'):
        ...
        # valid outputs
        sequence_output, label_ids, attention_mask = valid_sequence_output(
            sequence_output, label_ids, attention_mask, valid_mask, device)
