...
class NERData():
    ...
    def __init__(self, model_file="allenai/scibert_scivocab_uncased", scheme='IOBES'):
        ...
        '''
        # load tokenizer
        self.pre_tokenizer = MaterialsTextTokenizer(
            Path(__file__).resolve().parent.as_posix()+'/phraser.pkl')
    ...
    def get_classes(self, labels):
        ...
        classes = [
            '{}-{}'.format(p, c) for p in prefixes for c in classes_raw if c not in self.invalid_annotations]
    ...
    def filter_data(self, data):
        ...
        for i, entry in enumerate(tqdm(data, desc='| filtering entries |')):
            try:
                identifier = '{}/{}/{}'.format(entry['meta']['doi'], str(
                    entry['meta']['par']), str(entry['meta']['split']))
                d = {'meta': entry['meta']}
            except:
                try:
                    identifier = '{}/{}'.format(entry['meta']
                                                ['doi'], str(entry['meta']['par']))
                    d = {
                        'meta': {'doi': entry['meta']['doi'], 'par': entry['meta']['par'], 'split': 0}}
                except:
                    try:
                        identifier = entry['doi']
                        d = {
                            'meta': {'doi': entry['doi'], 'par': 0, 'split': 0}}
                    except:
                        try:
    ...
    def load_from_memory_annotated(self, data):
        ...
        # 변형1
        if 'ACTIVE_MATERIAL' in data[0][0]['labels']:
            _, data_filt = self.filter_data(data[0])

        id = 0
        # entry 1개는 1문단 'meta', 'tokens', 'labels' / data_filt는 전체 문단 개수 800개
        for entry in tqdm(data_filt, desc='| loading annotated entries |'):
            d = {'id': id, 'meta': entry['meta'], 'tokens': entry['tokens']}
            data_raw.append(d)
            # add labels in entry to raw label set entry['text'][0]['labels']
    ...
    def load_from_memory_unannotated(self, data):
        ...
        for entry in tqdm(data_filt, desc='| pre-tokenizing unannotated entries |'):
            ...
            try:
                ...
                try:
                    sents = [self.pre_tokenizer.process(
                        sent, convert_number=False, normalize_materials=False) for sent in entry['sents']]
                except:
                    sents = [self.pre_tokenizer.process(sent, convert_number=False, normalize_materials=False)
                             for sent in self.pre_tokenizer.tokenize(entry['text'], keep_sentences=True)]
            for tokens in sents:
                s = []
                for tok in tokens:
    ...
    def split_entries(self, data_raw, split_dict={'main': 1}, shuffle=False, seed=256):
        ...
        index_split_vals = (np.cumsum(split_vals) *
                            len(data_raw)).astype(np.uint32)
        # split data according to split indices
        data_split = {split_keys[i]: data_raw[:index_split_vals[i]] if i ==
                      0 else data_raw[index_split_vals[i-1]:index_split_vals[i]] for i in range(len(split_keys))}
        return data_split
    ...
    def format_entries(self, data_split):
        ...
        for split in data_split.keys():
            ...
            for d in data_split[split]:
                ...
                data_formatted[split].append({'id': d['id'], 'meta': d['meta'], 'tokens': [{key: [
                                             token[key] for token in sentence] for key in ['text', 'annotation']} for sentence in d['tokens']]})
    ...
    def insert_cls(self, d):
        ...
        d['token_ids'].insert(
            0, self.tokenizer.convert_tokens_to_ids(self.cls_dict['text']))
    ...
    def create_features(self, data_labeled):
        ...
        for split in data_labeled.keys():
            data_label_range = tqdm(
                data_labeled[split], desc='| writing {} features |'.format(split))
            # for example in dataset split
            for dat in data_label_range:
                # initialize empty dictionary for features
                d = {key: dat['id'] if key == 'id' else [] for key in [
                    'id', 'tokens', 'labels', 'token_ids', 'label_ids', 'attention_mask', 'valid_mask']}
                # for sentence in example
                for i in range(len(dat['tokens'])):
                    s = {key: [] for key in ['tokens', 'labels', 'token_ids',
                                             'label_ids', 'attention_mask', 'valid_mask']}
                    # for token in sentence
                    n_tokens = len(dat['tokens'][i]['text'])
                    for j in range(n_tokens):
                        # split token using bert tokenizer dat['tokens'][0]['text'][1] : 'electrochemical'
                        # self.tokenizer.tokenize(dat['tokens'][i]['text'][j]) : ['high']
                        # dat['tokens'][i]['text'][j] : 'High'
                        word_tokens = self.tokenizer.tokenize(
                            dat['tokens'][i]['text'][j])
                        ...
                        for k, word_token in enumerate(word_tokens):
                            ...
                            s['token_ids'].append(
                                self.tokenizer.convert_tokens_to_ids(word_token))
                            ...
                            if k == 0:
                                ...
                                # append the label
                                s['labels'].append(
                                    dat['tokens'][i]['label'][j])
                                # append the label id using the class dictionary
                                s['label_ids'].append(
                                    self.class_dict[dat['tokens'][i]['label'][j]])
                        ...
                        if j == len(dat['tokens'][i]['text'])-1:
                            ...
                            s['token_ids'].append(
                                self.tokenizer.convert_tokens_to_ids(self.sep_dict['text']))
                            s['label_ids'].append(
                                self.class_dict[self.sep_dict['label']])
                            s['attention_mask'].append(1)
                            s['valid_mask'].append(1)
    ...
    def split_entries_merge_sentences(self, data_feature, sentence_level):
        ...
        for split in data_feature.keys():
            data_label_range = tqdm(data_feature[split], desc=(
                '| splitting long {} paragraphs' if sentence_level else '| splitting {} paragraphs |').format(split))
            for dat in data_label_range:
                if sentence_level:
                    for i in range(len(dat['tokens'])):
                        d = {'id': dat['id'], 'pt': i}
                        d.update({key: dat[key][i] for key in [
                                 'tokens', 'labels', 'token_ids', 'label_ids', 'attention_mask', 'valid_mask']})
                        dat_split_feature[split].append(d)
                else:
                    slen = [len(s) for s in dat['tokens']]
                    tnum = sum(slen)
                    n_splits = int(
                        np.ceil((self.token_limit-np.sqrt(self.token_limit**2-4*tnum))/2))
                    n_shave = 0
                    if n_splits > 1:
                        valid = False
                        while not valid:
                            ...
                            for i in range(len(bounds)):
                                d = {'id': dat['id'], 'pt': i}
                                d.update({key: [v for s in dat[key][bounds[i][0]:bounds[i][1]] for v in s] for key in [
                                         'tokens', 'labels', 'token_ids', 'label_ids', 'attention_mask', 'valid_mask']})
                                self.insert_cls(d)
                                ds.append(d)
                                if len(d['tokens']) > ml:
                            ...
                            if ml > self.token_limit:
                                ...
                                if n_splits > len(dat['tokens']):
                                    ...
                                    if n_shave == len(slen):
                                        valid = True
                                        print(
                                            'failed to split paragraph on sentence breaks into subparagraphs below the token limit, skipping')
                                    else:
                                        d.update({key: [v for s in dat[key][:-n_shave] for v in s] for key in [
                                                 'tokens', 'labels', 'token_ids', 'label_ids', 'attention_mask', 'valid_mask']})
                        ...
                        d = {'id': dat['id'], 'pt': 0}
                        d.update({key: [v for s in dat[key] for v in s] for key in [
                                 'tokens', 'labels', 'token_ids', 'label_ids', 'attention_mask', 'valid_mask']})
                        self.insert_cls(d)
                        dat_split_feature[split].append(d)
    ...
    def pad_features(self, data_split_feature):
        ...
        for split in data_split_feature.keys():
            ...
            for dat in data_split_feature[split]:
                ...
                d['labels'].extend((max_length-length) *
                                   [self.pad_dict['label']])
                d['token_ids'].extend(
                    (max_length-length)*[self.tokenizer.convert_tokens_to_ids(self.pad_dict['text'])])
                d['label_ids'].extend(
                    (max_length-length)*[self.class_dict[self.pad_dict['label']]])
    ...
    def create_datasets(self, data_input_feature):
        ...
        for split in data_input_feature.keys():
            # collect features
            ids = torch.tensor([d['id'] for d in data_input_feature[split]],
                               dtype=torch.long, device=torch.device('cpu'))
            pts = torch.tensor([d['pt'] for d in data_input_feature[split]],
                               dtype=torch.uint8, device=torch.device('cpu'))
            token_ids = torch.tensor(
                [d['token_ids'] for d in data_input_feature[split]], dtype=torch.long, device=torch.device('cpu'))
            label_ids = torch.tensor(
                [d['label_ids'] for d in data_input_feature[split]], dtype=torch.uint8, device=torch.device('cpu'))
            attention_mask = torch.tensor(
                [d['attention_mask'] for d in data_input_feature[split]], dtype=torch.bool, device=torch.device('cpu'))
            valid_mask = torch.tensor(
                [d['valid_mask'] for d in data_input_feature[split]], dtype=torch.bool, device=torch.device('cpu'))
            # store as tensor dataset
            self.dataset[split] = TensorDataset(
                ids, pts, token_ids, label_ids, attention_mask, valid_mask)
    ...
    def preprocess(self, data, split_dict={'main': 1}, is_file=True, annotated=True, sentence_level=False, shuffle=False, seed=256):
        ...
        # creat datasets
        self.create_datasets(self.pad_features(self.split_entries_merge_sentences(self.create_features(
            self.label_entries(self.format_entries(self.split_entries(data, split_dict, shuffle, seed)))), sentence_level)))
    ...
    def create_dataloaders(self, batch_size=32, shuffle=True, seed=256):
        ...
        for split in self.dataset.keys():
            ...
            self.dataloaders[split] = DataLoader(
                self.dataset[split], batch_size=batch_size, shuffle=shuffle, num_workers=0, pin_memory=True)
